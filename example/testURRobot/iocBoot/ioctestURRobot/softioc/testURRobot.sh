...
# Manually set IOC_STARTUP_DIR if testURRobot.sh will reside somewhere other than softioc
#!IOC_STARTUP_DIR=/home/username/epics/ioc/synApps/testURRobot/iocBoot/ioctestURRobot/softioc

# Manually set IOC_COMMAND_DIR if testURRobot.sh will reside somewhere other than softioc
#!IOC_COMMAND_DIR=/home/username/epics/ioc/synApps/testURRobot/iocBoot/ioctestURRobot/softioc/commands

# Set EPICS_HOST_ARCH if the env var isn't already set properly for this IOC
#!EPICS_HOST_ARCH=linux-x86_64
#!EPICS_HOST_ARCH=linux-x86_64-debug

IOC_NAME=testURRobot
# The name of the IOC binary isn't necessarily the same as the name of the IOC
IOC_BINARY=testURRobot
# The absolute path to the directory containing the IOC's bin directory, if running an example IOC binary
#!IOC_BIN_DIR=/home/username/epics/synApps/support/testURRobot/bin
...
checkpid() {
    MY_UID=`${ID} -u`
    # The '\$' is needed in the pgrep pattern to select testURRobot, but not testURRobot.sh
